...
```{r}
tune_model_split <- function(df_train, df_test, models, metric="r2") {
...
```{r}
tune_model_cv <- function(df, models, nfolds, metric="r2") {
  n <- nrow(df)
...
df <- df_A %>% 
  dplyr::mutate(LogSales = log(AvgSales)) |>
  dplyr::select("LogSales", "CollegeTF", "Month", "Category", "Population", "City", "Year")
models <- list(
  c("CollegeTF"),
  c("CollegeTF", "Month"),
  c("CollegeTF", "Category"),
  ...
  c("CollegeTF", "Category", "Population"),
  c("CollegeTF", "Month", "Category", "Population"),
  c("CollegeTF", "Category", "Population", "Year"),
  c("CollegeTF", "Month", "Category", "Population", "Year")
)

order <- tune_model_cv(df, models, nfolds=5, metric="adjusted_r2")|>
...
```{r}
df <- df_B %>% 
  mutate(LogLiters = log(AvgLiters)) |>
  dplyr::select("LogLiters", "CollegeTF", "Category", "Population", "City")
models <- list(
  c("CollegeTF"),
  c("CollegeTF", "Category"),
  c("CollegeTF", "Population"),
...
order <- tune_model_cv(df, models, nfolds=5, metric="r2")|>
  arrange(desc(r2))
order
```
...
```{r}
predictions <- lm(log(AvgSales) ~ CollegeTF + Category + Population + Month, data = df)
